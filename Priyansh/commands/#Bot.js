...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
  ...
  var rand = tl[Math.floor(Math.random() * tl.length)]
    ...
    if ((event.body.toLowerCase() == "chutiya bot") || (event.body.toLowerCase() == "RKB")) {
     return api.sendMessage("BHAG MADRCHOD TERI MAA KO CHODU 😾 :))", threadID);
 BHOSDIKE  if ((event.body.toLowerCase() == "pagal bot") || (event.body.toLowerCase() == "pgl")) {
  ...
  if ((event.body.toLowerCase() == "🫣") || (event.body.toLowerCase() == "🫣🫣")) {
   ...
   if ((event.body.toLowerCase() == "") || (event.body.toLowerCase() == "")) {
     return api.sendMessage("💐🤘🤟  *★᭄𝗢𝘄𝗻𝗲𝗿 ཫ𒁍 ⃝𝐀𝐲𝐮𝐬𝐡🤘🤟💐", threadID);
   };
